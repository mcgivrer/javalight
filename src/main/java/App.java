...
public class App implements KeyListener {
    ...
    private void initScene() {
        ...
        addLight(new Light("light-sun").setLightType(LightType.POINT).setIntensity(100)
                .setPosition((world.getWidth()) / 5, (world.getHeight() - 40) / 6).setSize(80, 80)
                .setFillColor(new Color(0, 255, 230)));
        addLight(new Light("light-directional").setLightType(LightType.DIRECTIONAL).setIntensity(100)
                .setPosition((world.getWidth()) * 2 / 5, (world.getHeight() - 40) / 6).setSize(80, 80)
                .setFillColor(new Color(255, 255, 230)));
        addLight(new Light("light-spot").setLightType(LightType.SPOT).setIntensity(100)
                .setPosition((world.getWidth() * 3 / 5) / 2, world.getHeight()).setSize(50, world.getHeight())
                .setFillColor(new Color(255, 255, 250)));
        addLight(new Light("light-area").setLightType(LightType.AREA).setIntensity(100).setPosition(0, 0)
    ...
    private void render() {
        ...
        // g.fill(new Rectangle2D.Double(0, 0, world.getWidth(), world.getHeight()));
        lights.stream().filter(Entity::isActive).forEach(l -> {
            if (currentCamera != null) {
                g.translate(-currentCamera.position.getX(), -currentCamera.position.getY());
            }
            drawLight(g, (Light) l);
    ...
    private void drawLight(Graphics2D g, Light e) {
        g.setComposite(AlphaComposite.getInstance(AlphaComposite.SRC_OVER, (float) (e.getIntensity() / 255)));
        g.setColor(e.getFillColor());
        switch (e.getLightType()) {
        case LightType.POINT -> g.fill(
                new Ellipse2D.Double(e.getPosition().getX(), e.getPosition().getY(), e.getWidth(), e.getHeight()));
        case LightType.DIRECTIONAL -> g.fill(
                new Rectangle2D.Double(e.getPosition().getX(), e.getPosition().getY(), e.getWidth(), e.getHeight()));
        case LightType.SPOT -> g.fill(new Polygon(
